...
import { IconPencil, IconTrash, IconUsers, IconPlus } from '@tabler/icons-react'
        ...
        {filtered.length === 0 ? (
          ...
          <div>
            <div className="grid grid-cols-[1fr_1fr_auto] px-4 py-2.5 border-b border-[var(--color-border-light)] bg-[var(--color-bg)]">
              ...
              <span className="text-[10px] font-medium uppercase tracking-wider text-[var(--color-text-muted)]"></span>
            </div>
            {filtered.map((client) => (
              <div
                key={client.id}
                className="grid grid-cols-[1fr_1fr_auto] items-center px-4 py-3.5 border-b border-[var(--color-border-light)] last:border-b-0 hover:bg-[var(--color-bg)] transition-colors"
              >
                <div>
