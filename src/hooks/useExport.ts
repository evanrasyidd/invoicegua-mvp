...
import type { Document, Client, ServiceTemplate, Settings, RecurringInvoice } from '../db/database'
import { DEFAULT_SERVICE_TEMPLATES, DEFAULT_SETTINGS } from '../db/seeds'

export async function exportBackup(): Promise<void> {
  const [documents, clients, serviceTemplates, settings, recurring] = await Promise.all([
  ...
  const backup = {
    ...
    data: { documents, clients, serviceTemplates, settings, recurring },
  }
...
export async function importBackup(file: File): Promise<void> {
  ...
  const { documents, clients, serviceTemplates, settings, recurring } = backup.data

  await db.transaction(
    'rw',
    [db.documents, db.clients, db.serviceTemplates, db.settings, db.recurring],
    async () => {
      ...
      if (Array.isArray(recurring) && recurring.length) {
        await db.recurring.bulkAdd(recurring as RecurringInvoice[])
      }
    },
  )
}
